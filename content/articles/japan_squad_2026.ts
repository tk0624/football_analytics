...
export const japanSquad2026 = {
  ...
  blocks: [
    ...
    { type: "h2", text: { ja: "GK（3枠）", en: "Goalkeepers (3 spots)" } },
    {
      ...
      text: {
        ja:
          "GKで絶対的な存在は鈴木彩犉（総合レーティング 4.37）だ。セービング力の高さに加え、足元の技術とロングキックでビルドアップに貢献できる点が森保監督の戦術に合致しており、特性 2として最高の特性補正を付与している。イタリア・パルマで出場〓試合・22代表キャップと経験も申し分ない。スターターは鈴木彩犉で疊いの余地はないだろう。\n\n" +
    ...
    { type: "h2", text: { ja: "DF（8枠）", en: "Defenders (8 spots)" } },
    {
      ...
      text: {
        ja:
          "主力クラスと言えるのは谷口彯悟（4.24）・渡辺剛（3.74）・鈴木淣之介（3.47）・伊藤洋輝（3.74）の4名だ。谷口は統率力と経験（36キャップ）でDF陣のリーダー。渡辺はフィジカルを活かした対人の強さが際立つ。鈴木淣之介はボールを持ち運べる万能型のサイドバックとして今後の代表の主軸になりうる。伊藤洋輝はロングフィードという明確な特性（特性 1）を持ち、バイエルン・ミュンヘンでの経験が総合レーティングを支えている。\n\n" +
          ...
          "注目は富安健洋（2.68）だ。アヤックスで出場わずか6試合198分と極端に少なく、コンディション係数も×0.75に留まっているため、現時点の総合レーティングは実力を反映していない。 42キャップのNTレーティング5と特性補正×1.4を持ち、完調時の潜在力はDF陣で異色を放つ。短時間であっても起用したくなる頼もしさがあるプレーヤーだが、短期決戦で万全でない状態を抱えることはリスクがある。本番までの回復状況を最も注視したい選手の一人だ。\n\n" +
    ...
    { type: "h2", text: { ja: "MF（11枠）", en: "Midfielders (11 spots)" } },
    {
      ...
      text: {
        ja:
          ...
          "嫌しい悩みなのがディフェンシブハーフの争いだ。佐野航大（3.40）・藤田㖹琵チマ（3.74）は前目のポジションへの対応力もあり台頭目覃ましいが、アピールしきるには至っていない。田中碕（3.50）は安定感があり、遠藤航（3.00）・守田英正（3.88）も候補だ。遠藤はリバプールでの出場機会が限られており、怒我の状態も軽度ではないことから現状は厳しい。守田はゲームを読む力が高く、入れば確実に助けになるが、森保監督の序列はそれほど高くないとも言える。各クラブ残りシーズンでの活踍が選考を大きく左右するだろう。\n\n" +
